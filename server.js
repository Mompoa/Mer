...
const FOOD_COUNT = 256;
const VIRUS_COUNT = 10;
const TICK_RATE = 50;
const MAX_BLOBS = 64;

// Game modes
// ffa: standard, 10 second merge timer (~200 ticks at 50ms)
// ffa-fast: fast merge, 2 second merge timer (~40 ticks)
const MODE_MERGE_TIMER = { "ffa": 200, "ffa-fast": 30 };
const MODE_LABELS = { "ffa": "FFA", "ffa-fast": "FFA FAST" };
...
function createRoom(code, roomName, maxPlayers, mode) {
  const MAP_WIDTH = 1500, MAP_HEIGHT = 1500;
  mode = (mode === "ffa-fast") ? "ffa-fast" : "ffa";
  const food = new Map();
...
createRoom("PUBLIC", "Public Room", 30, "ffa");
...
function ejectMass(player) {
  ...
  player.blobs.forEach(blob => {
    if (blob.r < 25) return;
    // Use last known direction or eject upward if no target
    const dx = player.targetX !== null ? player.targetX - blob.x : 0;
    const dy = player.targetY !== null ? player.targetY - blob.y : -1;
    const len = Math.hypot(dx, dy) || 1;
    const nx = dx/len, ny = dy/len;
    const ejectR = 11;
    blob.r = massToRadius(Math.PI * blob.r * blob.r - Math.PI * ejectR * ejectR);
    const ejId = genId();
    // Fast and visible eject mass
    player.room.food.set(ejId, {
      id: ejId,
      x: blob.x + nx*(blob.r+ejectR+2), y: blob.y + ny*(blob.r+ejectR+2),
      r: ejectR, color: player.color,
      vx: nx*14, vy: ny*14, life: 60,
    });
  });
}

function splitPlayer(player) {
  if (player.splitCooldown > 0 || player.blobs.length >= MAX_BLOBS) return;
  player.splitCooldown = 10;
  const mergeTimer = MODE_MERGE_TIMER[player.room.mode] || 200;
  [...player.blobs].filter(b => b.r > 20).forEach(blob => {
    ...
    player.blobs.push({ id: genId(), x: blob.x+nx*blob.r, y: blob.y+ny*blob.r, r: blob.r, vx: nx*28, vy: ny*28, mergeTimer });
  });
}
...
wss.on("connection", ws => {
  ...
  ws.on("message", raw => {
    ...
    if (msg.type === "createRoom") {
      ...
      const mode = msg.mode === "ffa-fast" ? "ffa-fast" : "ffa";
      const room = createRoom(code, roomName, maxP, mode);
      sendWs(ws, { type: "roomCreated", code, name: room.name, maxPlayers: room.maxPlayers, mode: room.mode });
      broadcastRoomList();
      return;
    }
    ...
    if (msg.type === "move") { player.targetX = msg.x; player.targetY = msg.y; }
    if (msg.type === "stop") { player.targetX = null; player.targetY = null; }
    ...
    for (const [pid, player] of players) {
      ...
      player.blobs.forEach(blob => {
        if (player.targetX !== null) {
          // Each blob moves independently toward the player's target
          const dx = player.targetX - blob.x, dy = player.targetY - blob.y;
          const len = Math.hypot(dx, dy) || 1;
          // Normal walking speed, slows slightly as blob grows
          const speed = Math.max(1.5, 5 - blob.r * 0.02);
          if (len > 3) {
            blob.vx = (dx/len) * speed;
            blob.vy = (dy/len) * speed;
          } else {
            blob.vx = 0;
            blob.vy = 0;
          }
      ...
      if (blobs.length > 1) {
        for (let i = 0; i < blobs.length; i++) {
          for (let j = i+1; j < blobs.length; j++) {
            ...
            if (!canMerge) {
              // Push apart if overlapping while timer still active
              const minD = a.r + b.r;
              if (d < minD && d > 0.01) {
              ...
              // Timer done — pull blobs toward each other to ensure they merge
              if (d > 0.01) {
                const nx = (b.x-a.x)/d, ny = (b.y-a.y)/d;
                const pullSpeed = Math.min(3, d * 0.15);
                a.x += nx*pullSpeed; a.y += ny*pullSpeed;
                b.x -= nx*pullSpeed; b.y -= ny*pullSpeed;
              }
              // Merge once overlapping
