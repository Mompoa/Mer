...
function getRoomList() {
  ...
  for (const [, room] of rooms) {
    ...
    list.push({ code: room.code, name: room.name, players: room.players.size, maxPlayers: room.maxPlayers });
  }
  return list;
}

function broadcastRoomList() {
  const msg = JSON.stringify({ type: "roomList", rooms: getRoomList() });
  for (const client of wss.clients) {
    if (client.readyState === WebSocket.OPEN) client.send(msg);
  }
}

function createRoom(code, name, maxPlayers, hidden) {
  ...
  const room = {
    code,
    name: (name || code).slice(0, 24),
    maxPlayers: Math.min(Math.max(parseInt(maxPlayers) || 10, 2), 30),
    hidden: !!hidden,
    mapWidth: MAP_WIDTH,
...
wss.on("connection", (ws) => {
  ...
  ws.on("message", (raw) => {
    ...
    if (msg.type === "createRoom") {
      ...
      const room = createRoom(code, msg.name, msg.maxPlayers, msg.hidden);
    ...
    if (msg.type === "join") {
      ...
      if (code && rooms.has(code)) {
        ...
        if (room.players.size >= room.maxPlayers) {
          sendWs(ws, { type: "error", text: "Room is full!" });
          return;
        }
      } else {
        if (!rooms.has("PUBLIC")) createRoom("PUBLIC", "Public Room", 30, false);
